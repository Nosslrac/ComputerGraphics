...
void initialize()
{
	...
	glBufferData(GL_ARRAY_BUFFER, labhelper::array_length(pos2) * sizeof(float), pos2, GL_STATIC_DRAW);

	glGenBuffers(1, &texcoordBuffer2);
	glBindBuffer(GL_ARRAY_BUFFER, texcoordBuffer2);
	glBufferData(GL_ARRAY_BUFFER, labhelper::array_length(explosionTexture) * sizeof(float), explosionTexture,
		GL_STATIC_DRAW);
...
void gui()
{
	...
	// Magnification.
	auto magnif = mag == 0 ? GL_NEAREST : GL_LINEAR;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magnif);
